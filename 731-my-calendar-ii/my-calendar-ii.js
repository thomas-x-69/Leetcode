...
MyCalendarTwo.prototype.book = function (start, end) {
    ...
    for (let i = 0; i < this.events.length; i++) {
        if (end > this.events[i][0] && start < this.events[i][1]) {
            const overlap = this.findOverlap(this.events[i], [start, end])
...
/**
 * @param {Array<[number, number]>} region1
 * @param {Array<[number, number]>} region2
 * @return {Array<[number, number]>}
 */
MyCalendarTwo.prototype.findOverlap = function (region1, region2) {
